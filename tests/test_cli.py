...
def test_github_token_from_env_var():
    """Test that GitHub token is auto-detected from GITHUB_TOKEN environment variable."""
    runner = CliRunner()
    ...
    with patch('simple_repo_downloader.cli.GitHubClient') as MockClient:
        ...
        # Run with GITHUB_TOKEN environment variable set
        result = runner.invoke(cli, [
            'download', 'github', 'testuser'
        ], env={'GITHUB_TOKEN': 'ghp_test_token'})

        # Verify GitHubClient was initialized with the token from env var
        MockClient.assert_called_once()
        call_kwargs = MockClient.call_args[1]
        assert call_kwargs['token'] == 'ghp_test_token'


def test_gitlab_token_from_env_var():
    """Test that GitLab token is auto-detected from GITLAB_TOKEN environment variable."""
    runner = CliRunner()
    ...
    with patch('simple_repo_downloader.cli.GitLabClient') as MockClient:
        mock_client = AsyncMock()
        mock_client.list_repositories = mock_list_repositories
        MockClient.return_value = mock_client

        # Run with GITLAB_TOKEN environment variable set
        result = runner.invoke(cli, [
            'download', 'gitlab', 'testuser'
        ], env={'GITLAB_TOKEN': 'glpat_test_token'})

        # Verify GitLabClient was initialized with the token from env var
        MockClient.assert_called_once()
        call_kwargs = MockClient.call_args[1]
        assert call_kwargs['token'] == 'glpat_test_token'
